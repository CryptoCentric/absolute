...
void CMasternode::Check(bool fForce)
{
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        // part 1: expire based on absoluted ping
    ...
    // Works on mainnet/testnet only and not the case on regtest/povnet.
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos, CConnman& connman)
{
    ...
    pmn->Check(true);
    // relay ping for nodes in ENABLED/EXPIRED/NO_SENTINEL_PING state only, skip everyone else
