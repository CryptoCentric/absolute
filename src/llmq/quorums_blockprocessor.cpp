...
bool CQuorumBlockProcessor::ProcessBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& state)
{
    ...
    bool fAIP0003Active = VersionBitsState(pindex->pprev, Params().GetConsensus(), Consensus::DEPLOYMENT_AIP0003, versionbitscache) == THRESHOLD_ACTIVE;
    if (!fAIP0003Active) {
        return true;
    }
...
bool CQuorumBlockProcessor::GetCommitmentsFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, std::map<Consensus::LLMQType, CFinalCommitment>& ret, CValidationState& state)
{
    ...
    bool fAIP0003Active = VersionBitsState(pindexPrev, consensus, Consensus::DEPLOYMENT_AIP0003, versionbitscache) == THRESHOLD_ACTIVE;
...
bool CQuorumBlockProcessor::IsCommitmentRequired(Consensus::LLMQType llmqType, int nHeight)
{
    ...
    {
        // TODO We added the commitments code while AIP3 was already activated on testnet and we want
        ...
        // AIP3 chain.
        // As we need to fork/revert the chain later to re-test all deployment stages of AIP3, we can
        // remove all this temporary code later.
        LOCK(cs_main);
        const auto& consensus = Params().GetConsensus();
        if (consensus.nTemporaryTestnetForkAIP3Height != 0 &&
            nHeight > consensus.nTemporaryTestnetForkAIP3Height &&
