    ...
    if(nBlockHeight >= consensusParams.nBudgetPaymentsStartBlock &&
       nOffset < consensusParams.nBudgetPaymentsWindowBlocks) {
        ...
        if(masternodeSync.IsSynced()) {
            ...
            // testnet/povnet/regtest should produce regular blocks only
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::%s -- ERROR: Missing required payment, possible payees: '%s', amount: %f ABSOLUTE\n", __func__, strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
