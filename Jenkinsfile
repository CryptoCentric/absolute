...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      // restore cache
      try {
        copyArtifacts(projectName: "absolutepay-absolute/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
      } catch (Exception e) {
      }
      if (fileExists("ci-cache-${target}.tar.gz")) {
        hasCache = true
        echo "Using cache from absolutepay-absolute/${BRANCH_NAME}"
      } else {
        try {
          copyArtifacts(projectName: 'absolutepay-absolute/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
        } catch (Exception e) {
        }
        if (fileExists("ci-cache-${target}.tar.gz")) {
          hasCache = true
          echo "Using cache from absolutepay-absolute/develop"
        }
      }
