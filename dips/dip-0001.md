...
Implementation of AIP1 allows users to actively participate in increasing the capacity of Absolute.
...
For this reason this AIP changes consensus rules so that blocks with a transaction over 100kB are ruled invalid and orphaned.
...
We reserve the last bit in the version to signal for AIP1 activation.
A AIP0001 enabled miner will broadcast block version `0x20000002` only if the masternode selected for that block is also AIP1 enabled.
...
Note that blocks mined by non upgraded miners will broadcast version `0x20000000` without regard for the AIP1 acceptance of the masternode selected.

As in BIP9 each block is assigned a state according to the flow chart:

![State Flow Chart](/aip-0001/states.png)
...
This translates to 80% of the AIP1 flags.

### Expected activation conditions.

Given that bit1 will be 0 if mined by a non-AIP1 miner this actually targets activation in the following cases.
...
AIP1 as outlined above only relies on an inspection of the chain to determine the activation block height.
